...
output_dir = "heightmap_diffusion_checkpoints"
...
def generate_heightmap_image(input_image_path, vae, unet, scheduler, device, target_image_size, output_path="generated_heightmap.png", num_inference_steps=50):
    ...
    generator = torch.Generator(device=torch_device_obj).manual_seed(42)
    initial_noise = torch.randn(latent_shape, generator=generator, device=torch_device_obj)
...
if __name__ == "__main__":
    ...
    target_image_size = (args.image_height, args.image_width)
    ...
    output_heightmap_image_path = args.output_heightmap_image_path
    ...
    if is_training and dataloader is not None and optimizer is not None and lr_scheduler is not None and \
       (device == 'cpu' or (device == 'cuda' and scaler is not None)):
        ...
        if is_training: 
            ...
            if is_training :
                print("\n모델 학습 완료!")
                final_model_dir = os.path.join(current_run_output_dir, "final_model")
