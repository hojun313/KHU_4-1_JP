...
# 학습 관련 설정
batch_size = 16 # GPU VRAM 크기에 맞춰 배치 크기 설정
num_workers = 0 # 데이터 로딩 워커 수 (Windows는 0으로 설정 권장)
num_epochs = 100 # 총 학습 에폭 수

use_data_augmentation = False # 데이터 증강 사용 여부 설정
...
# 체크포인트 설정
output_dir = "완료 모델 창고" # 체크포인트 저장 디렉토리 (하이트맵용으로 이름 변경)
