...
def train_fn(loader, model, optimizer, loss_fn, scaler, device):
    """한 에포크 동안의 학습을 담당하고, 평균 손실을 반환합니다."""
    ...
    for batch_idx, (data, targets) in enumerate(loop):
        ...
        targets = targets[:, :1, :, :].to(device=device) # 하이트맵은 1채널 사용
        
        # Mixed precision training
        # CPU에서는 autocast와 GradScaler가 다르게 동작하거나 필요 없을 수 있음
        ...
        if device_type != 'cpu':
            ...
            scaler.update()
        else: # CPU에서는 scaler 사용 안 함
            loss.backward()
            optimizer.step()

        running_loss += loss.item() * data.size(0) # 각 배치의 손실에 실제 배치 크기를 곱함
        num_samples += data.size(0)
        loop.set_postfix(loss=loss.item())
    
    epoch_loss = running_loss / num_samples if num_samples > 0 else 0.0
    print(f"에포크 평균 손실: {epoch_loss}")
    return epoch_loss # 스케줄러에 전달할 평균 손실 반환
...
def main(args):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        avg_epoch_loss = train_fn(loader, model, optimizer, loss_fn, scaler, DEVICE)
